...
function Projects() {
  return (
    <div className="overflow-hidden h-full w-full lg:w-[90vw]">
      <div className="grid grid-cols-1 lg:grid-cols-3 w-full h-full overflow-y-scroll">
        {projects.map(({ id, title, des, img, iconLists, github, link }) => (
          <div
            key={id}
            className="h-[360px] w-full lg:h-[50vh] lg:w-[30vw] scale-95 flex flex-col items-center justify-center gap-5 transition duration-300 rounded-3xl bg-gradient-to-r from-[#DE8359]/[0.4] to-[#D86063]/[0.1] hover:shadow-[0_8px_16px_rgb(255_255_255/0.6)] overflow-hidden"
          >
            <span className="absolute top-0 flex flex-col gap-2">
              <Image
                src={img}
                alt={title}
                className="z-10 object-cover h-[200px] w-[100vw]"
                height={300}
                width={500}
              />

              <span>
                <h1 className="px-1 font-bold lg:text-xl md:text-lg text-whtite text-base line-clamp-1 uppercase">
                  {title}
                </h1>
                <p className="text-justify px-2 font-light text-sm md:text-md text-orange-200/[0.8]">
                  {des}
                </p>
              </span>
            </span>
