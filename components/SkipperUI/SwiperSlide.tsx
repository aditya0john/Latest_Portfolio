...
export const CardCarousel: React.FC<ProjectProps> = ({
  project,
  autoplayDelay = 3000,
  showPagination = true,
  showNavigation = true,
        ...
        <div className="w-full">
          ...
          >
            {project.map((item) => (
              <SwiperSlide
                key={item.id}
                className="h-[360px] w-[full] lg:h-[50vh] lg:w-[30vw] scale-95 flex flex-col items-center justify-center gap-5 transition duration-300 rounded-3xl bg-gradient-to-br from-[#E7B493] to-[#FFEFEB]"
              >
                <span className="absolute top-0 flex flex-col gap-6">
                  <Image
                    src={item.img}
                    alt={item.title}
                    className="z-10 object-cover h-[300px] w-[100vw] rounded-xl"
                    height={300}
                    width={500}
                  />

                  <span className="opacity-70 px-4">
                    <h1 className="font-bold lg:text-4xl md:text-lg noise bg-clip-text text-transparent text-base line-clamp-1 uppercase">
                      {item.title}
                    </h1>
                    <p className="text-justify max-w-xl font-semibold text-sm md:text-lg text-black/[0.6]">
                      {item.des}
                    </p>
                  </span>
                </span>

                <div className="w-full px-2 flex absolute bottom-2 justify-between">
                  <div className="flex">
